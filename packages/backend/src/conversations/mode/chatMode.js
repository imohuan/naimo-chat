...
async function processChatMode(context) {
  ...
  // 加载系统提示词并替换变量
  const systemPrompts = replaceVariablesInMessages(chatModePrompt, {});

  // 添加系统提示词
  messages.push(...systemPrompts);

  // 添加历史消息（限制最近 10 条，避免上下文过大）
  if (conversation && conversation.messages) {
    const historyMessages = conversation.messages
      .filter((msg) => msg.role !== "system" && !msg.isRequesting) // 过滤系统消息和正在请求的消息
      .slice(-10) // 只取最近 10 条
      .map((msg) => ({
        role: msg.role,
        content: msg.content,
...
module.exports = processChatMode;
