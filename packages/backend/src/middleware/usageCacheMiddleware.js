 ...
 */
function createUsageCacheMiddleware(
  sessionUsageCache,
  config = null,
  agentsManager = null
) {
          ...
          if (agent.shouldHandle && agent.shouldHandle(req, config)) {
            ...
            if (agent.reqHandler) {
              try {
                ...
                console.error(
                  `[usageCacheMiddleware] Agent ${agent.name} reqHandler 执行失败:`,
                  error
                );
              }
            }
              ...
              try {
                ...
                if (
                  ...
                  try {
                    ...
                    console.log(
                      `[usageCacheMiddleware] 工具执行完成: ${currentToolName}, toolMessages.length: ${toolMessages.length}, assistantMessages.length: ${assistantMessages.length}`
                    );
                ...
                if (data.event === "message_delta") {
                  ...
                  if (hasToolMessages) {
                    console.log(
                      `[usageCacheMiddleware] 检测到工具消息，stop_reason: ${stopReason}, toolMessages.length: ${toolMessages.length}, assistantMessages.length: ${assistantMessages.length}`
                    );
                    ...
                    if (req.headers["x-request-id"]) {
                      ...
                      const [requestId, suffix] = req.headers["x-request-id"].split(
                        char_separator
                      );
                      const newSuffix = parseInt(suffix || "0", 10) + 1;
                      headers["X-Request-Id"] =
                        requestId + char_separator + newSuffix.toString();
                    }
