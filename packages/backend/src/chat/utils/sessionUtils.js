...
class SessionManager {
   ...
   */
  sendEvent(streamingId, payload) {
    const session = this.sessions.get(streamingId);
    if (!session) return;
  ...
  closeSession(streamingId, code) {
    ...
    setTimeout(() => this.sessions.delete(streamingId), this.SESSION_TTL_MS);
  }
   ...
   */
  createSession(streamingId, child = null) {
    const session = {
      child,
      clients: new Set(),
      events: [],
    ...
    this.sessions.set(streamingId, session);
    return session;
  }
   ...
   */
  getSession(streamingId) {
    return this.sessions.get(streamingId);
  }
   ...
   * @returns {boolean}
   */
  hasSession(streamingId) {
    return this.sessions.has(streamingId);
  }
}
