...
// 直接硬编码（等同 run_base.ps1），可被外部环境覆盖
const DEFAULT_ENV = {
  API_TIMEOUT_MS: "300000",
  ANTHROPIC_BASE_URL: "http://127.0.0.1:3457/",
  ANTHROPIC_AUTH_TOKEN: "sk-imohuan",
};
...
async function handleChatStart(req, res) {
  try {
    ...
    if (useMock) {
      return handleChatTestStart(res, userMessage);
    }
    ...
    child.stdout?.setEncoding("utf8");
    child.stdout?.on("data", (chunk) => {
      console.log("[chat:start] stdout chunk", {
        streamingId,
        length: chunk.length,
        snippet: chunk.toString().slice(0, 200),
      });
...
function handlePermissionDecision(res, pathname, body) {
...
async function ensureMcpConfigFile(streamingId) {
  ...
  const config = {
    mcpServers: {
      ...
      // },
      "demo-permissions": {
        // Claude CLI 的 schema 对 HTTP 服务器要求使用 transport 字段
        // type: "streamable-http",
        // url: "http://localhost:3457/mcp/claude-permissions",
        command: "npx",
        args: ["mcp-remote", "http://localhost:3457/mcp/claude-permissions"],
        env: {
          APPROVAL_ENDPOINT_BASE: base,
          MCP_STREAMING_ID: streamingId, // 关键：将 streamingId 传递给 MCP 服务器
        },
...
function handleChatTestStart(res, _userMessage) {
  const streamingId = randomUUID();
  const session = {
    child: null,
    clients: new Set(),
  ...
  // 预设事件序列：system init -> task 进度 -> 思考流 -> 编辑代码 -> 总结 -> 结束
  const events = [
...
const server = http.createServer(async (req, res) => {
  ...
  if (
    ...
    const body = await readRequestBody(req);
    handlePermissionDecision(res, pathname, body);
    return;
  }
