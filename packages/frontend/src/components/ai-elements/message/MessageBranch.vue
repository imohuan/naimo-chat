...
function goToPrevious() {
  ...
  const next =
    currentBranch.value > 0 ? currentBranch.value - 1 : totalBranches.value - 1;
...
function goToNext() {
  if (totalBranches.value === 0) return;
  const next =
    currentBranch.value < totalBranches.value - 1 ? currentBranch.value + 1 : 0;
  handleBranchChange(next);
}
...
provide(MessageBranchWidthKey, containerWidth);
</script>
