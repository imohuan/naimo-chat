    ...
    <div v-if="show"
      ...
      <div
        ...
        <div class="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50 shrink-0">
          <h3 class="font-bold text-slate-800 flex items-center gap-2">
            ...
            <span class="text-sm font-normal text-slate-600 ml-2">
              ...
              <span v-if="mcpTools.length > 0">，{{ mcpTools.length }} 个 MCP</span>)
            </span>
          </h3>
        ...
        <div class="flex-1 overflow-hidden flex">
          <!-- 左侧工具列表 -->
          <div class="w-80 border-r border-slate-100 bg-slate-50 overflow-hidden flex flex-col shrink-0">
            <!-- 搜索框和筛选器 -->
            <div class="p-4 border-b border-slate-200 shrink-0 space-y-3">
              <div class="relative">
                <SearchOutlined class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                <input v-model="searchKeyword" type="text" placeholder="搜索工具..."
              ...
              <div class="flex gap-2">
                <button @click="toolTypeFilter = 'all'"
                  class="flex-1 px-3 py-1.5 text-xs font-medium rounded-md transition-all" :class="toolTypeFilter === 'all'
                      ? 'bg-indigo-600 text-white shadow-sm'
                      : 'bg-white text-slate-600 border border-slate-200 hover:bg-slate-50'
                    ">
                  全部 ({{ allTools.length }})
                </button>
                <button @click="toolTypeFilter = 'mcp'"
                  class="flex-1 px-3 py-1.5 text-xs font-medium rounded-md transition-all" :class="toolTypeFilter === 'mcp'
                      ? 'bg-purple-600 text-white shadow-sm'
                      : 'bg-white text-purple-600 border border-purple-200 hover:bg-purple-50'
                    ">
                  MCP ({{ mcpTools.length }})
                </button>
                <button @click="toolTypeFilter = 'base'"
                  class="flex-1 px-3 py-1.5 text-xs font-medium rounded-md transition-all" :class="toolTypeFilter === 'base'
            ...
            <div class="flex-1 overflow-y-auto p-4 space-y-2">
              <div v-for="(tool, index) in filteredTools" :key="index"
                class="p-4 rounded-lg cursor-pointer transition-all border relative overflow-hidden" :class="selectedToolIndex === index
                ...
                <div class="flex items-start justify-between gap-2">
                  <div class="flex-1 min-w-0">
                    <div class="mb-1">
                      <div class="font-mono text-sm font-bold text-slate-800 truncate"
                        :title="formatToolName(tool.name)">
                        {{ formatToolName(tool.name) }}
                      </div>
                    </div>
                    <div class="text-xs text-slate-500 mt-1 line-clamp-2" v-if="tool.description">
                      {{ tool.description }}
                    </div>
                    <div class="flex items-center gap-2 mt-2 flex-wrap">
                      <span class="text-[10px] px-2 py-0.5 rounded bg-slate-100 text-slate-700 font-mono">
              ...
              <div v-if="filteredTools.length === 0" class="text-center py-8 text-slate-400 text-sm">
          ...
          <div class="flex-1 overflow-hidden flex flex-col p-6">
            <div v-if="selectedToolIndex === null" class="flex-1 flex items-center justify-center">
            ...
            <div v-else-if="selectedTool" class="flex-1 overflow-hidden flex flex-col">
              <!-- Tab 导航 -->
              <nav class="flex items-center gap-6 border-b border-slate-200 mb-4 shrink-0">
                <button class="px-1 py-3 text-sm font-medium transition-all flex items-center gap-2 relative -mb-px"
                  :class="currentDetailTab === 'info'
                ...
                <button v-if="Object.keys(toolProperties).length > 0"
                  class="px-1 py-3 text-sm font-medium transition-all flex items-center gap-2 relative -mb-px" :class="currentDetailTab === 'params'
                      ? 'text-primary border-b-2 border-primary'
                      : 'text-slate-500 hover:text-slate-700 border-b-2 border-transparent'
              ...
              <div class="flex-1 overflow-hidden flex flex-col">
                <!-- 基本信息 Tab -->
                <div v-if="currentDetailTab === 'info'" class="flex-1 overflow-y-auto">
                  <div class="space-y-3">
                    <div class="flex items-center gap-2 mb-3">
                      <span class="text-xs px-3 py-1 rounded font-bold" :class="isMCPTool(selectedTool.name)
                          ? 'bg-purple-200 text-purple-800'
                          : 'bg-blue-200 text-blue-800'
                        ">
                        {{
                    ...
                    <div>
                      ...
                      <div class="font-mono text-sm font-bold text-slate-800 select-text">
                    ...
                    <div v-if="selectedTool.description">
                      ...
                      <div class="text-sm text-slate-700 whitespace-pre-wrap select-text">
                        {{ selectedTool.description }}
                      </div>
                    </div>
                    <div v-if="Object.keys(toolProperties).length === 0" class="mt-4 pt-4">
                ...
                " class="flex-1 overflow-y-auto">
                  <div class="space-y-4">
                    <div v-for="(prop, key) in toolProperties" :key="key" class="p-4">
                      <div class="flex items-center gap-2 mb-2 select-text">
                        <span class="font-mono text-sm font-bold text-slate-800">
                          {{ key }}
                        </span>
                        <span v-if="toolRequired.includes(key)"
                          class="text-xs px-2 py-0.5 rounded bg-red-100 text-red-700 font-bold">
                          必填
                        </span>
                        <span v-if="prop.type" class="text-xs px-2 py-0.5 rounded bg-blue-100 text-blue-700 font-mono">
                          {{ prop.type }}
                        </span>
                      </div>
                      <div v-if="prop.description" class="text-sm text-slate-600 mb-2 select-text">
                        {{ prop.description }}
                      </div>
                      ...
                      <div v-if="prop.default !== undefined" class="text-xs text-slate-500">
                        默认值:
                      ...
                      <details v-if="Object.keys(prop).length > 5" class="mt-2">
                        ...
                        <pre class="mt-2 text-xs font-mono p-2 overflow-x-auto">{{ formatJson(prop) }}</pre>
                      </details>
                    </div>
                  </div>
                </div>
                ...
                <!-- 完整 JSON Tab -->
                <div v-if="currentDetailTab === 'json'" class="flex-1 min-h-0 flex flex-col">
                  <div class="overflow-hidden flex-1 min-h-0">
                    <CodeEditor class="h-full" :model-value="formatJson(selectedTool)" language="json" :readonly="true"
                      :options="{
                        wordWrap: 'on',
