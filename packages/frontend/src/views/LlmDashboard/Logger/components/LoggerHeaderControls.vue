    ...
    <div class="flex items-center gap-2">
      ...
      <template v-if="activeMode === 'logs'">
        <div class="relative" data-log-dropdown @click.stop>
          <button @click="logDropdownOpen = !logDropdownOpen" type="button"
            ...
            <div class="flex items-center gap-2 flex-1 min-w-0">
              <DescriptionOutlined class="absolute left-2.5 w-4 h-4 text-slate-400 shrink-0" />
              <span v-if="selectedLogFileObj" class="truncate text-xs">
                ...
                <div class="text-[9px] text-slate-500 font-medium leading-tight align-right">
                  {{ selectedLogFileObj.name }}
                </div>
              </span>
              <span v-else class="text-slate-400 text-xs">选择日志文件</span>
            </div>
            <ExpandMoreOutlined :class="[
          ...
          <div v-if="logDropdownOpen"
            ...
            <div v-for="file in logFiles" :key="file.path" @click="
            ...
            <div v-if="logFiles.length === 0" class="px-4 pt-4 pb-2 text-sm text-slate-400 text-center">
              暂无日志文件
            </div>
          </div>
        </div>
        ...
        <!-- 刷新按钮 -->
        <RefreshButton v-if="selectedLogFileObj" :is-refreshing="isRefreshing" @refresh="() => emit('refresh')" />

        <!-- 清空当前日志按钮 -->
        <button v-if="selectedLogFileObj" @click="emit('clearLog')" type="button"
      ...
      <template v-if="activeMode === 'messages'">
        <!-- 刷新按钮 -->
        <RefreshButton :is-refreshing="isRefreshingMessages" @refresh="() => emit('refreshMessages')" />
      ...
      <!-- MCP 工具调用模式刷新按钮 -->
      <template v-if="activeMode === 'mcp-tools'">
        <!-- 刷新按钮 -->
        <RefreshButton :is-refreshing="isRefreshingMcpTools" @refresh="() => emit('refreshMcpTools')" />
