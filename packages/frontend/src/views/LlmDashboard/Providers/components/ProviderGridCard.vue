...
<template>
  ...
  >
    ...
    <div class="space-y-2">
      ...
      <!-- 模型列表：只显示一行，不允许换行，显示全部模型 -->
      <div class="flex gap-1.5 overflow-x-auto pb-1">
        <span
          v-for="m in provider.models || []"
          :key="m"
          class="text-[10px] px-2 py-0.5 bg-white border border-slate-200 rounded text-slate-600 font-mono cursor-pointer hover:bg-gray-100 hover:text-primary-700 transition-colors whitespace-nowrap shrink-0"
